...
def forward(inputs, targets, memory):
    ...
    hs, cs, xs, wes, zs, f_gates, i_gates, o_gates, cs_hat, os, ps, ys = {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}
    ...
    for t in range(len(inputs)):
        ...
        f_gates[t] = sigmoid(np.dot(Wf, zs[t]) + bf)

        # compute the input gate
        # i_gate = sigmoid (W_i \cdot [h X] + b_i)
        i_gates[t] = sigmoid(np.dot(Wi, zs[t]) + bi)
        ...
        cs[t] = f_gates[t] * cs[t - 1] + i_gates[t] * cs_hat[t]

        # output gate
        # o_gate = sigmoid (Wo \cdot [h X] + b_o)
        o_gates[t] = sigmoid(np.dot(Wo, zs[t]) + bo)

        # new hidden state for the LSTM
        # h = o_gate * tanh(c_new)
        hs[t] = o_gates[t] * np.tanh(cs[t])
    ...
    activations = (hs, cs, xs, wes, zs, f_gates, i_gates, o_gates, cs_hat, os, ps, ys)
...
def backward(activations, clipping=True):
    ...
    hs, cs, xs, wes, zs, f_gates, i_gates, o_gates, cs_hat, os, ps, ys = activations
    ...
    for t in reversed(range(len(inputs))):
        ...
        do_gate_net = do_gate * dsigmoid(o_gates[t])
        dbo += do_gate_net
        dWo += np.dot(do_gate_net, zs[t].T)
        dz += np.dot(Wo.T, do_gate_net)

        # Memory
        dc = dh * o_gates[t] * dtanh(cs[t]) + dcnext
        dcnext = dc * f_gates[t]

        # C_hat layer
        dc_hat = dc * i_gates[t]
        dc_hat_net = dc_hat * dtanh(cs_hat[t])
        dbc += dc_hat_net
        dWc += np.dot(dc_hat_net, zs[t].T)
        dz += np.dot(Wc.T, dc_hat_net)

        # Input gate
        di_gate = dc * cs_hat[t]
        di_gate_net = di_gate * dsigmoid(i_gates[t])
        dbi += di_gate_net
        dWi += np.dot(di_gate_net, zs[t].T)
        dz += np.dot(Wi.T, di_gate_net)

        # Forget gate
        df_gate = dc * cs[t-1]
        df_gate_net = df_gate * dsigmoid(f_gates[t])
        dbf += df_gate_net
        dWf += np.dot(df_gate_net, zs[t].T)
        dz += np.dot(Wf.T, df_gate_net)

        # Embedding layer
        dhnext, dwe = dz[:dhnext.shape[0]], dz[dhnext.shape[0]:]
